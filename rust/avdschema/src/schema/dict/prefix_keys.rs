...
/// `PrefixKeys` represents keys like `custom_structured_configuration`_*.
...
impl<'a> Dict {
        ...
        M: SchemaDataMapping<'input>,
    {
        ...
        let resolved_dict_keys = self.resolve_dict_keys(dict, store, None);
        let result = dict
            .keys()
...
mod tests {
    ...
    fn get_prefix_keys_basic() {
        ...
        let dict_schema = Dict {
            prefix_keys: Some(vec![PrefixKeys {
                prefixes_key: Some("custom_structured_configuration_prefixes".to_owned()),
                prefixes: None,
                include_suffix_in_data: false,
                schema_ref: "eos_cli_config_gen#/keys/key2".to_owned(),
            }]),
            ..Default::default()
        };
    ...
    fn get_prefix_keys_with_suffix() {
        ...
        let dict_schema = Dict {
            prefix_keys: Some(vec![PrefixKeys {
                prefixes_key: None,
                prefixes: Some(vec!["custom_structured_configuration_".to_owned()]),
                include_suffix_in_data: true,
                schema_ref: "eos_cli_config_gen#/keys/suffix_dict".to_owned(),
            }]),
            ..Default::default()
        };
    ...
    fn get_prefix_keys_with_default() {
        ...
        let dict_schema = Dict {
            keys: Some(OrderMap::from_iter([(
                "custom_structured_configuration_prefixes".to_owned(),
                List {
