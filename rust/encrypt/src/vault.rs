...
//! Ansible Vault v1.2 compatible encryption and decryption.
//!
//! Format:
//! ```text
...
pub fn vault_encrypt(plaintext: &[u8], password: &str, vault_id: &str) -> Result<String, EncryptError> {
    ...
    let header = format!("$ANSIBLE_VAULT;1.2;AES256;{}", vault_id);
    Ok(format!("{}\n{}\n", header, lines.join("\n")))
}

/// Decrypt data from Ansible Vault v1.2 format.
///
/// Returns the decrypted plaintext and the vault ID from the header.
pub fn vault_decrypt(vault_data: &str, password: &str) -> Result<(Vec<u8>, String), EncryptError> {
...
mod tests {
    ...
    fn vault_encrypt_decrypt_roundtrip() {
        ...
        let encrypted = vault_encrypt(plaintext, password, vault_id).expect("Encryption failed");
    ...
    fn vault_decrypt_wrong_password_fails() {
        ...
        let encrypted = vault_encrypt(plaintext, password, vault_id).unwrap();
    ...
    #[test]
    fn vault_empty_vault_id() {
        let plaintext = b"data";
        let password = "pass";
        let vault_id = "";

        let encrypted = vault_encrypt(plaintext, password, vault_id).unwrap();
        assert!(encrypted.starts_with("$ANSIBLE_VAULT;1.2;AES256;\n"));

        let (decrypted, returned_vault_id) = vault_decrypt(&encrypted, password).unwrap();
        assert_eq!(decrypted, plaintext);
        assert_eq!(returned_vault_id, "");
    }
